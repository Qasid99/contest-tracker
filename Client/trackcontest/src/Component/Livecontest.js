...
export default function ContestApi(props) {
    ...
    for (let i = 0; i < textFromStorage.objects.length; i++) {
        ...
        if (textFromStorage.objects[i].resource_id == siteid) {

            if (session == "live") {
                if (dateCheck(start_date, end_date, current_date) == 1) {
                ...
                else if (dateCheck(start_date, end_date, current_date) == 0) {
                    if (1) {
                        const obj = {
                            event: `${textFromStorage.objects[i].event}`,
                            href: `${textFromStorage.objects[i].href}`,
                ...
                else if (dateCheck(start_date, end_date, current_date) == -1) {
